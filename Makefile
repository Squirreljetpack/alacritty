...
APP_NAME = CommandSpace.app
APP_TEMPLATE = $(ASSETS_DIR)/osx/Alacritty.app
...
app: $(APP_NAME)-native ## Create a CommandSpace.app
app-universal: $(APP_NAME)-universal ## Create a universal CommandSpace.app
$(APP_NAME)-%: $(TARGET)-%
	@rm -rf $(APP_DIR)/$(APP_NAME)
	@mkdir -p $(APP_DIR)
	@cp -fRp $(APP_TEMPLATE) $(APP_DIR)
	@mv $(APP_DIR)/Alacritty.app $(APP_DIR)/$(APP_NAME)
...
dmg-universal: $(DMG_NAME)-universal ## Create a universal CommandSpace.dmg
$(DMG_NAME)-%: $(APP_NAME)-%
	@echo "Packing disk image..."
	@rm -f $(DMG_DIR)/$(DMG_NAME)
	@ln -sf /Applications $(DMG_DIR)/Applications
	@hdiutil create $(DMG_DIR)/$(DMG_NAME) \
		-volname "CommandSpace" \
		-fs HFS+ \
		-srcfolder $(APP_DIR) \
		-ov -format UDZO
